...
- **Variable Frequenz und Periode** über `frequencyMax` und `periodMax` — „1 bis 2 x täglich", „alle 2 bis 3 Tage". Beide Achsen gleichzeitig zu variieren ist unzulässig (`TimingVarFreqOrPeriod`). Bei einer reinen Intervallangabe darf zudem nur eine der beiden Achsen größer als `1` sein (`TimingFreqOrPeriodGtOne`): „2 x alle 8 Stunden" wird als „alle 4 Stunden" ausgedrückt. Im generischen DE-Profil lösen beide Regeln eine Warnung aus.
